"""
Ensemble all bake-off models by averaging softmax probabilities on val.

Each model uses its own tokenizer; we tokenize per-model, predict probabilities,
then average. Optionally weights each model by its solo macro F1.

Outputs:
  - models/ensemble/ensemble_report.txt
  - models/ensemble/per_model_predictions.npy (saved softmax stacks)

Usage:
    py src/ensemble_eval.py
    py src/ensemble_eval.py --weighted   # weight by per-model val macro F1
"""
...
def model_macro_f1(short_name):
    """Read per-model val macro F1 from bakeoff_results.csv."""
...
def main():
    ...
    ap.add_argument("--weighted", action="store_true", help="weight by per-model val macro F1")
    ...
    if not final_dirs:
        print(f"No models found under {BAKEOFF_DIR}/*_final")
        return
    print(f"Models found: {[d.name for d in final_dirs]}")
    ...
    for k, d in enumerate(final_dirs):
        ...
        if args.weighted:
            ...
            print(f"  weight (val mF1) = {weights[k]:.4f}")

    if args.weighted:
        weights = weights / weights.sum()
    else:
        weights = weights / weights.sum()  # uniform
    ...
    print()
    print(f"Ensemble val accuracy:    {acc:.4f}")
    print(f"Ensemble val weighted F1: {f1_w:.4f}")
    print(f"Ensemble val macro F1:    {f1_m:.4f}")
    print(f"Min class F1:             {f1_per_class.min():.4f}")
    print()
    print(report)

    ENSEMBLE_DIR.mkdir(parents=True, exist_ok=True)
    np.save(ENSEMBLE_DIR / "per_model_probs.npy", stack)
    np.save(ENSEMBLE_DIR / "weights.npy", weights)
    with open(ENSEMBLE_DIR / "ensemble_report.txt", "w", encoding="utf-8") as f:
        ...
        f.write(report)
    print(f"Saved -> {ENSEMBLE_DIR}/ensemble_report.txt")
